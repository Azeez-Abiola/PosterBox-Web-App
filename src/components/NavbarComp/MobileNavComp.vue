<template>
  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 transition-opacity duration-300"
    :class="isVisible ? 'opacity-100' : 'opacity-0'" @click="handleBackdropClick"></div>
  ...
  <div
    ...
    <div class="flex items-center justify-between p-6 border-b border-gray-100">
      <div class="flex items-center space-x-3">
        <img src="/images/logo-red.png" alt="PosterBox logo" class="h-10 w-auto" />
        <!-- Brand Name -->
        <span class="font-bold text-lg text-black">PosterBox</span>
      </div>

      <button @click="handleClose" class="p-2 rounded-lg hover:bg-gray-100 transition-colors duration-200">
    ...
    <div class="flex flex-col h-full">
      ...
      <nav class="flex-1 px-6 py-6">
        <ul class="space-y-2">
          <li v-for="(link, index) in navlinks" :key="link.title" class="transform transition-all duration-300"
            :style="{ transitionDelay: `${isVisible ? index * 100 + 150 : 0}ms` }"
            :class="isVisible ? 'translate-x-0 opacity-100' : 'translate-x-8 opacity-0'">
            <RouterLink :to="link.route" @click="handleNavClick(link.route)" :class="[
              ...
              <i class="pi pi-angle-right text-gray-400 group-hover:text-red-600 transition-colors duration-200 transform group-hover:translate-x-1"
                :class="isRouteActive(link.route) ? 'text-red-600' : ''"></i>
            </RouterLink>
          </li>
        </ul>
      </nav>
